#!/usr/bin/env python3
"""
ADC Configurator (Tkinter) - following standardized GUI structure
Converted from original ADC script to match CAN configurator pattern.
"""
import os
...
class AdcChannelModel:
    ...
    AdcChannelHighLimit: int = 0
    AdcChannelLowLimit: int = 0
    AdcChannelLimitCheck: bool = False
    AdcChannelRangeSelect: str = "ADC_RANGE_ALWAYS"  # Dropdown: Multiple range options
...
class AdcGroupModel:
    ...
    AdcGroupReplacement: str = "ADC_GROUP_REPL_ABORT_RESTART"  # Dropdown: Multiple replacement options
    AdcGroupTriggSrc: str = "ADC_TRIGG_SRC_SW"  # Dropdown: ADC_TRIGG_SRC_HW, ADC_TRIGG_SRC_SW
    AdcHwTrigSignal: str = "ADC_HW_TRIG_BOTH_EDGES"  # Dropdown: Multiple trigger signal options
...
@dataclass
class AdcAppModel:
    output_filepath: str = os.path.abspath(os.path.join("output", "adc_config.arxml"))
    AdcConfigSet: bool = True
...
class AdcArxmlExporter:
    ...
    def export(model: AdcAppModel, filepath: str):
        ...
        ET.SubElement(cs, "SHORT-NAME").text = "AdcConfigSet"
        ET.SubElement(cs, "DEFINITION-REF", {"DEST":"ECUC-PARAM-CONF-CONTAINER-DEF"}).text = "/AUTOSAR/EcucDefs/Adc/AdcConfigSet"
        p = ET.SubElement(cs, "PARAMETER-VALUES")
        pv = ET.SubElement(p, "ECUC-BOOLEAN-PARAM-VALUE")
        ET.SubElement(pv, "SHORT-NAME").text = "AdcConfigSetIncluded"
        ET.SubElement(pv, "VALUE").text = "true" if model.AdcConfigSet else "false"
...
class AdcConfiguratorApp(tk.Frame):
    ...
    def _build_ui(self):
        ...
        # Build sections
        self._build_section_AdcConfigSet(self.content)
    ...
    def _build_section_AdcConfigSet(self, parent):
        sec = CollapsibleFrame(parent, text="📦 AdcConfigSet")
        sec.pack(fill="x", pady=4)
        self.var_AdcConfigSet = tk.BooleanVar(value=self.model.AdcConfigSet)
        ttk.Checkbutton(sec.content, text="Include AdcConfigSet", variable=self.var_AdcConfigSet).pack(anchor="w")
    ...
    def on_generate(self):
        ...
        self.model.AdcConfigSet = bool(self.var_AdcConfigSet.get())
